...
export const MatchingViewing = ({ question, points, number, compact }: MatchingViewingProps) => {
  return (
    <div className={cn(compact ? 'space-y-2' : 'space-y-4')}>
      ...
      {question.data.shufflePairs && (
        <div className={cn('flex items-center', compact ? 'gap-1' : 'gap-2')}>
          <Label className={cn('font-medium', compact ? 'text-xs' : 'text-sm')}>Matching Pairs</Label>
      ...
      <div className={cn('grid grid-cols-1 md:grid-cols-2', compact ? 'gap-2' : 'gap-4')}>
        ...
        <div className={cn(compact ? 'space-y-1' : 'space-y-1.5')}>
          <h4 className={cn('font-semibold', compact ? 'text-xs' : 'text-sm')}>Column A</h4>
        ...
        {/* Right Column */}
        <div className={cn(compact ? 'space-y-1' : 'space-y-1.5')}>
          <h4 className={cn('font-semibold', compact ? 'text-xs' : 'text-sm')}>Column B</h4>
          {question.data.pairs.map((pair, index) => (
            <div
              key={`right-${pair.id}`}
              className={cn(
