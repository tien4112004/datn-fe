import type {
  AssessmentMatrixApiService,
  CreateAssessmentMatrixRequest,
  UpdateAssessmentMatrixRequest,
  CreateTopicRequest,
  UpdateTopicRequest,
} from '@/features/assessment-matrix/types/service';
import type { AssessmentMatrix, Topic, MatrixCell, SubjectCode } from '@/features/assessment-matrix/types';
...
const createSampleMatrix = (id: string, index: number): AssessmentMatrix => {
  ...
  return {
    id,
    name: `${subjectCode === 'T' ? 'Math' : subjectCode === 'TV' ? 'Vietnamese' : 'English'} Assessment Matrix ${index + 1}`,
...
export default class AssessmentMatrixMockApiService implements AssessmentMatrixApiService {
  private matrices: AssessmentMatrix[];
  ...
  async getMatrixById(id: string): Promise<AssessmentMatrix> {
  ...
  async createMatrix(data: CreateAssessmentMatrixRequest): Promise<AssessmentMatrix> {
    await new Promise((resolve) => setTimeout(resolve, 300));

    const newMatrix: AssessmentMatrix = {
      ...data.matrix,
      id: generateId(),
  ...
  async updateMatrix(id: string, data: UpdateAssessmentMatrixRequest): Promise<AssessmentMatrix> {
    await new Promise((resolve) => setTimeout(resolve, 300));
    ...
    const updated: AssessmentMatrix = {
      ...this.matrices[index],
      ...data.matrix,
      updatedAt: new Date().toISOString(),
    };
