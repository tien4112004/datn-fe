...
export default class MindmapMockService implements MindmapApiService {
  ...
  async generateMindmap(
    ...
    return new Promise((resolve) => {
      setTimeout(() => {
        // Generate a simple AI node structure based on the request
        const maxDepth = request.maxDepth || 3;
        const maxBranches = request.maxBranchesPerNode || 5;

        const generateChildren = (depth: number, parentTopic: string): AiGeneratedNode[] => {
          if (depth >= maxDepth) return [];

          const numChildren = Math.min(Math.floor(Math.random() * 3) + 2, maxBranches);
          return Array.from({ length: numChildren }, (_, i) => ({
            content: `${parentTopic} - Subtopic ${i + 1}`,
            children: generateChildren(depth + 1, `${parentTopic} - Subtopic ${i + 1}`),
          }));
        };

        const generated: AiGeneratedNode = {
          content: request.topic,
          children: generateChildren(1, request.topic),
        };
