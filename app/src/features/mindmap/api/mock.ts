...
export default class MindmapMockService implements MindmapApiService {
  ...
  async generateMindmap(request: import('../types/service').MindmapGenerateRequest): Promise<Mindmap> {
    return new Promise((resolve) => {
      setTimeout(() => {
        const generated: Mindmap = {
          id: crypto.randomUUID(),
          title: `AI: ${request.topic}`,
          description: `Generated by ${request.provider} - ${request.model}`,
          thumbnail: '',
          metadata: { direction: 'horizontal', forceLayout: false },
          nodes: [
            {
              id: 'root',
              type: MINDMAP_TYPES.ROOT_NODE,
