export default {
  navigation: {
    sidebar: {
      slogan: 'Trợ thủ đắc lực của giáo viên',
      notification: 'Thông báo',
      features: 'Tính năng',
      projects: 'Dự án',
      files: 'Tập tin',
      classes: 'Lớp học',
      questionBank: 'Kho câu hỏi',
      subjects: 'Môn học',
      schedules: 'Lịch',
      home: 'Trang chủ',
      admin: 'Quản Trị',
      questionRendererDemo: 'Demo Trình Hiển Thị Câu Hỏi',
      buyCoins: 'Mua Coins',
    },
  },
  pages: {
    presentations: 'Bài trình bày',
    notFound: '404 Không tìm thấy',
    pageNotFound: 'Không tìm thấy trang',
    pageNotFoundDescription: 'Xin lỗi, trang bạn đang tìm kiếm không tồn tại hoặc có thể đã được di chuyển.',
    goBack: 'Quay lại',
    goHome: 'Về trang chủ',
    classes: 'Các lớp học',
    examMatrices: 'Ma Trận Đề Thi',
    createMatrix: 'Tạo Ma Trận',
    editMatrix: 'Chỉnh Sửa Ma Trận',
  },
  status: {
    active: 'Hoạt động',
    inactive: 'Không hoạt động',
  },
  permission: {
    viewer: 'Người xem',
    commenter: 'Người bình luận',
    editor: 'Người chỉnh sửa',
  },
  actions: {
    view: 'Xem',
    edit: 'Chỉnh sửa',
    delete: 'Xóa',
    close: 'Đóng',
  },
  pagination: {
    previous: 'Trước',
    next: 'Tiếp',
  },
  table: {
    presentation: {
      id: 'Mã',
      thumbnail: 'Hình',
      title: 'Tiêu đề',
      createdAt: 'Ngày tạo',
      updatedAt: 'Cập nhật gần nhất',
      lastModified: 'Cập nhật gần nhất',
      emptyState: 'Không có bài trình bày nào được tìm thấy',
      searchPlaceholder: 'Tìm kiếm theo tiêu đề...',
      renameFileDialogTitle: 'Đổi tên bài trình bày',
      renameSuccess: 'Đổi tên bài thuyết trình "{{filename}}" thành công',
      renameDuplicatedMessage: 'Đã tồn tại bài trình bày với tên này',
      untitled: 'Bài trình bày chưa đặt tên',
      createFirst: 'Tạo bài trình bày đầu tiên của bạn để bắt đầu',
      presentation: 'bài trình bày',
      deleteTitle: 'Xóa bài trình bày',
      deleteMessage: 'Bạn có chắc chắn muốn xóa "{{name}}"? Hành động này không thể hoàn tác.',
      deleteSuccess: 'Đã xóa bài trình bày "{{name}}" thành công',
      deleteError: 'Xóa bài trình bày thất bại',
    },
    mindmap: {
      id: 'Mã',
      thumbnail: 'Hình',
      title: 'Tiêu đề',
      createdAt: 'Ngày tạo',
      updatedAt: 'Cập nhật gần nhất',
      lastModified: 'Cập nhật gần nhất',
      emptyState: 'Không có bản đồ tư duy nào được tìm thấy',
      searchPlaceholder: 'Tìm kiếm theo tên bản đồ tư duy...',
      renameFileDialogTitle: 'Đổi tên bản đồ tư duy',
      renameSuccess: 'Đổi tên bản đồ tư duy "{{filename}}" thành công',
      renameDuplicatedMessage: 'Đã tồn tại bản đồ tư duy với tên này',
      untitled: 'Bản đồ tư duy chưa đặt tên',
      createFirst: 'Tạo bản đồ tư duy đầu tiên của bạn để bắt đầu',
      deleteTitle: 'Xóa bản đồ tư duy',
      deleteMessage: 'Bạn có chắc chắn muốn xóa "{{name}}"? Hành động này không thể hoàn tác.',
      deleteSuccess: 'Đã xóa bản đồ tư duy "{{name}}" thành công',
      deleteError: 'Xóa bản đồ tư duy thất bại',
    },
    assignment: {
      id: 'Mã',
      title: 'Tiêu đề',
      status: 'Trạng thái',
      dueDate: 'Hạn nộp',
      createdAt: 'Ngày tạo',
      updatedAt: 'Cập nhật gần nhất',
      lastModified: 'Cập nhật gần nhất',
      emptyState: 'Không có bài tập nào',
      searchPlaceholder: 'Tìm kiếm bài tập...',
      renameFileDialogTitle: 'Đổi tên bài tập',
      renameSuccess: 'Đã đổi tên bài tập thành công',
      renameDuplicatedMessage: 'Đã tồn tại bài tập với tên này',
      untitled: 'Bài tập chưa đặt tên',
      createFirst: 'Tạo bài tập đầu tiên của bạn để bắt đầu',
      deleteTitle: 'Xóa bài tập',
      deleteMessage: 'Bạn có chắc chắn muốn xóa "{{name}}"? Hành động này không thể hoàn tác.',
      deleteSuccess: 'Đã xóa bài tập "{{name}}" thành công',
      deleteError: 'Xóa bài tập thất bại',
      statusDraft: 'Bản nháp',
      statusPublished: 'Đã xuất bản',
      statusArchived: 'Đã lưu trữ',
      due: 'Hạn',
    },
    sharedResources: {
      thumbnail: 'Hình',
      title: 'Tiêu đề',
      type: 'Loại',
      owner: 'Chủ sở hữu',
      permission: 'Quyền',
      permissionRead: 'Xem',
      permissionComment: 'Bình luận',
      permissionEdit: 'Chỉnh sửa',
      emptyState: 'Không có tài nguyên được chia sẻ',
      emptyStateHint: 'Tài nguyên được chia sẻ với bạn sẽ xuất hiện ở đây',
      searchPlaceholder: 'Tìm kiếm tài nguyên được chia sẻ...',
      untitled: 'Chưa đặt tên',
      sharedBy: 'Chia sẻ bởi',
    },
    actions: 'Hành động',
    renameError: 'Đổi tên thất bại',
    confirm: 'Xác nhận',
    cancel: 'Hủy',
    deleting: 'Đang xóa...',
    pagination: {
      rowsPerPage: 'Số hàng mỗi trang',
      showing: 'Hiển thị',
      to: 'đến',
      of: 'trên',
      entries: 'mục',
      goToFirstPage: 'Đi đến trang đầu',
      goToPreviousPage: 'Đi đến trang trước',
      goToPage: 'Đi đến trang',
      goToNextPage: 'Đi đến trang tiếp theo',
      goToLastPage: 'Đi đến trang cuối',
      firstPage: 'Đầu',
      previous: 'Trước',
      next: 'Tiếp',
      lastPage: 'Cuối',
    },
  },
  model: {
    placeholder: 'Chọn mô hình?',
    label: 'Mô hình',
    loading: 'Đang tải mô hình...',
    error: 'Lỗi khi tải mô hình',
  },
  unsavedChanges: {
    title: 'Thay đổi chưa được lưu',
    description: 'Bạn có thay đổi chưa được lưu. Bạn có chắc muốn rời đi? Các thay đổi của bạn sẽ bị mất.',
    stay: 'Ở lại',
    leave: 'Rời đi',
  },
  smallScreen: {
    title: 'Nên sử dụng màn hình lớn hơn',
    description:
      'Trình soạn thảo này được tối ưu hóa cho màn hình lớn hơn. Để có trải nghiệm tốt nhất, vui lòng sử dụng thiết bị có màn hình lớn hơn hoặc phóng to cửa sổ trình duyệt.',
    understand: 'Tôi hiểu',
  },
  presentation: {
    saveSuccess: 'Lưu bài trình bày thành công',
    saveFailed: 'Lưu bài trình bày thất bại',
    processFailed: 'Xử lý bài trình bày thất bại',
  },
  subjectNames: {
    math: 'Toán',
    vietnamese: 'Tiếng Việt',
    english: 'Tiếng Anh',
  },
  grade: 'Lớp',
  subject: 'Môn học',
  topics: 'Chủ đề',
  characters: 'ký tự',
};
