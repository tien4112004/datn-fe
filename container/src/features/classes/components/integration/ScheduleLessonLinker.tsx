...
const ScheduleLessonLinker = ({
  ...
  const availableLessons = useMemo(() => {
    ...
    return lessonPlans.filter((lesson) => {
      ...
      // Check if lesson matches the period's subject and time
      const matchesSubject = lesson.subject === selectedPeriod.subject;
      const matchesTime =
        lesson.startTime === selectedPeriod.startTime && lesson.endTime === selectedPeriod.endTime;

      return !isLinkedToOtherPeriod && matchesSubject && matchesTime;
    });
    ...
    <div className="space-y-6">
                  ...
                  <div key={lesson.id} className="rounded-lg border p-3">
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-muted-foreground text-sm">
                          {lesson.subject} • {format(new Date(lesson.date), 'PPP', { locale: vi })} •
      ...
      <Dialog open={isLinkDialogOpen} onOpenChange={setIsLinkDialogOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
              {t('linkLessonToPeriod')} - {selectedPeriod?.subject}
            </DialogTitle>
          </DialogHeader>
          <div className="space-y-4">
            {/* Period Details */}
            {selectedPeriod && (
              <div className="rounded bg-gray-50 p-3">
                ...
                <div className="grid grid-cols-2 gap-2 text-sm">
                  <div>
                    {t('subject')}: {selectedPeriod.subject}
                  </div>
                  <div>
                      ...
                      <SelectItem key={lesson.id} value={lesson.id}>
                        <div className="flex flex-col">
                          ...
                          <span className="text-muted-foreground text-xs">
                            {format(new Date(lesson.date), 'PPP', { locale: vi })}
                          </span>
                        </div>
                      </SelectItem>
                    ))}
...
export default ScheduleLessonLinker;
