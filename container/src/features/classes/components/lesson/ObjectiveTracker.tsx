...
interface ObjectiveTrackerProps {
  ...
  onAddNote: (objectiveId: string, note: string) => Promise<void>;
  canEdit?: boolean;
}
...
const ObjectiveTracker = ({
  ...
  const handleToggleAchieved = async (objectiveId: string, isAchieved: boolean) => {
    ...
    try {
      await onUpdateObjective(objectiveId, { isAchieved });
    } finally {
      setIsUpdating((prev) => {
  ...
  const handleSaveNote = async (objectiveId: string) => {
    ...
    setIsUpdating((prev) => new Set(prev).add(objectiveId));
    try {
      await onAddNote(objectiveId, note);
                    ...
                    <CollapsibleContent>
                      <div className="border-t bg-gray-50 px-4 pb-4">
                        <div className="space-y-3 pt-4">
                            ...
                            {isEditingNote ? (
                              <div className="space-y-2">
                                <Textarea
                                  ...
                                  onChange={(e) =>
                                    setEditingNotes((prev) => new Map(prev).set(objective.id, e.target.value))
                                  }
                          ...
                          {/* Metadata */}
                          <div className="text-muted-foreground text-xs">
                            {t('createdAt')}: {new Date(objective.createdAt).toLocaleString('vi-VN')}
                          </div>
                        </div>
                      </div>
                    </CollapsibleContent>
