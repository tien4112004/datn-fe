...
/**
 * Left-Only Layout Strategy
 *
 * All children extend to the left of their parent node.
 * This creates a classic tree view layout flowing right-to-left.
 *
 * Characteristics:
 * - Children stack vertically to the left of parent
 * - Sibling order determined by Y position (top to bottom)
 * - Uses left handle for source, right handle for target
 * - All nodes assigned to 'left' side regardless of creation method
 */
export class LeftOnlyLayoutStrategy extends BaseLayoutStrategy {
  readonly type: LayoutType = LAYOUT_TYPE.LEFT_ONLY;
   ...
   * Children are placed to the left of their parent, stacked vertically.
   */
    ...
    hierarchy.each((node: D3HierarchyNode) => {
      ...
      if (originalNode) {
        layoutedNodes.push({
          ...
          // Ensure all nodes are assigned to 'left' side for this layout
          data: {
            ...originalNode.data,
            side: originalNode.type === MINDMAP_TYPES.ROOT_NODE ? SIDE.MID : SIDE.LEFT,
          },
        });
      }
    });
  ...
  getEdgeHandles(_parentNode: MindMapNode, _childNode: MindMapNode): EdgeHandleInfo {
    // For left-only layout: parent's left handle to child's right handle
    return {
      sourceHandle: 'left',
      targetHandle: 'right',
    };
  ...
  getDefaultChildSide(_parentNode: MindMapNode, _existingChildren: MindMapNode[]): Side {
    // All children go to the left in this layout
    return SIDE.LEFT;
  }
}
 ...
 * Singleton instance
 */
export const leftOnlyLayoutStrategy = new LeftOnlyLayoutStrategy();
