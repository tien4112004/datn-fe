...
/**
 * Top Only Layout Strategy
 *
 * Children are positioned above their parent, centered horizontally.
 * This creates an upside-down organizational chart layout flowing bottom-to-top.
 *
 * Characteristics:
 * - Children stack horizontally above parent, centered
 * - Sibling order determined by X position (left to right)
 * - Uses top handle for source, bottom handle for target
 * - All nodes assigned to TOP side
 */
export class TopOnlyLayoutStrategy extends BaseLayoutStrategy {
  readonly type: LayoutType = LAYOUT_TYPE.TOP_ONLY;
   ...
   * Children are placed above their parent, centered horizontally.
   * All siblings share the same bottom edge (aligned below).
   */
  private positionHierarchy(
    ...
    hierarchy.descendants().forEach((node: D3HierarchyNode) => {
      ...
      if (originalNode.id === rootNode.id) {
        ...
        // All siblings will share the same bottom edge (y + height)
        // This will be adjusted later to align all siblings
        node.y = node.parent.y - node.parent.height - verticalSpacing - node.height;
        ...
        if (node.parent.children) {
          ...
          if (siblings.length === 1) {
            // Single child: center above parent
    ...
    // Align all siblings by their bottom edges
    const alignSiblingsByBottomEdge = (node: D3HierarchyNode) => {
      if (!node.children || node.children.length === 0) return;

      const siblings = node.children;
      // Find the maximum bottom edge (minimum y value since y increases downward, but we want them above)
      // For top layout, we want the lowest child (highest y) to determine the alignment
      const maxBottomY = Math.max(...siblings.map((child) => child.y + child.height));

      // Adjust all siblings so their bottom edges align
      siblings.forEach((child) => {
        child.y = maxBottomY - child.height;
      });

      // Recursively align grandchildren
    ...
    hierarchy.each((node: D3HierarchyNode) => {
      ...
      if (originalNode) {
        layoutedNodes.push({
          ...
          data: {
            ...originalNode.data,
            // Use TOP side for all non-root nodes
            side: originalNode.type === MINDMAP_TYPES.ROOT_NODE ? SIDE.MID : SIDE.TOP,
          },
        });
      }
    });
  ...
  getEdgeHandles(_parentNode: MindMapNode, _childNode: MindMapNode): EdgeHandleInfo {
    // For top only: parent's top handle to child's bottom handle
    return {
      sourceHandle: 'top',
      targetHandle: 'bottom',
    };
 ...
 * Singleton instance
 */
export const topOnlyLayoutStrategy = new TopOnlyLayoutStrategy();
