...
vi.mock("@/shared/components/ui/select", () => ({
  ...
  SelectLabel: ({ children, ...props }: any) => <div data-testid="select-label" {...props}>{children}</div>,
...
describe("CreateOutlinePage", () => {
  ...
  it("renders all main elements", () => {
    ...
    expect(screen.getAllByTestId("card-title").some(el => el.textContent === "examplePromptTitle")).toBeTruthy();
  });
  ...
  it("hides example prompts when prompt input is not empty", () => {
    ...
    expect(screen.queryAllByTestId("card-title").some(el => el.textContent === "examplePromptTitle")).toBeFalsy();
    for (let i = 1; i <= 6; i++) {
  ...
  it("renders all select inputs and allows selection", () => {
    render(<CreateOutlinePage />);
    // Slide count select
    expect(screen.getAllByTestId("select-value").some(el => el.textContent === "slideCountPlaceholder")).toBeTruthy();
    expect(screen.getAllByTestId("select-label").some(el => el.textContent === "slideCountLabel")).toBeTruthy();
    expect(screen.getAllByTestId("select-item").some(el => el.textContent === "1 slideCountUnit")).toBeTruthy();
    // Style select
    expect(screen.getAllByTestId("select-value").some(el => el.textContent === "stylePlaceholder")).toBeTruthy();
    expect(screen.getAllByTestId("select-label").some(el => el.textContent === "styleLabel")).toBeTruthy();
    expect(screen.getAllByTestId("select-item").some(el => el.textContent === "styleBusiness")).toBeTruthy();
    // Model select
    expect(screen.getAllByTestId("select-value").some(el => el.textContent === "modelPlaceholder")).toBeTruthy();
    ...
    expect(screen.getAllByTestId("select-item").some(el => el.textContent === "Gemini 2.0 Flash")).toBeTruthy();
  });
});
