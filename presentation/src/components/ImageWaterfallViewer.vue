<template>
  <div class="image-waterfall-viewer" ref="waterfallRef" :style="{ height: outerHeight + 'px' }">
    <div
...
const initLayout = () => {
  ...
  columnCount.value = Math.max(
    Math.floor((width + props.columnSpacing) / (props.columnWidth + props.columnSpacing)),
    1
  );
