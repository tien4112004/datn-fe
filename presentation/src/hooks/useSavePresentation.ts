...
import { useSaveStore, useContainerStore } from '@/store';
...
interface SavePresentationData {
  title: string;
  slides: Slide[];
  theme: SlideTheme;
...
export function useSavePresentation(presentationId: string, pinia: Pinia) {
  ...
  /**
   * Save presentation with all data (title, slides, theme, viewport, thumbnail)
   * Dispatches events to React for UI feedback
   */
  async function savePresentation(data: SavePresentationData): Promise<void> {
    try {
