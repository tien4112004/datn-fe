...
export default () => {
  ...
  // Horizontal uniform arrangement
  const uniformHorizontalDisplay = () => {
    ...
    // Get the sets of normal elements and group elements separately, and record the range of each item
    ...
    // Combine the sets of normal elements and group elements, then sort each item by position (from left to right)
    const list: Item[] = [...singleElemetList, ...formatedGroupList];
    list.sort((itemA, itemB) => itemA.min - itemB.min);

    // Calculate the interval required for uniform distribution of elements:
    ...
    // Calculate the target position of each element in order by position
    // The element in the first item is the starting point, no calculation needed
    // From the second item, the position of each item should be: previous item position + previous item width + interval
    // Note: the calculated position (pos) here is not the final left value of the element, but the minimum value of the target position range (the left value after element rotation ≠ minimum value of the range)
    ...
    // For rotated elements, it is necessary to calculate the offset of left before and after rotation for correction
  ...
  // Vertical uniform arrangement (logic is similar to the horizontal uniform arrangement method)
