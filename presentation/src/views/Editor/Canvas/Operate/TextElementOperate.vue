  ...
  <div class="text-element-operate">
    ...
    <template v-if="handlerVisible">
      <ResizeHandler
        ...
        :style="point.style"
        @mousedown.stop="($event: any) => scaleElement($event, elementInfo, point.direction)"
      />
      <RotateHandler
        class="operate-rotate-handler"
        :style="{ left: scaleWidth / 2 + 'px' }"
        @mousedown.stop="($event: any) => rotateElement($event, elementInfo)"
      />
    </template>
  </div>
</template>
...
const resizeHandlers = computed(() =>
  props.elementInfo.vertical ? verticalTextElementResizeHandlers.value : textElementResizeHandlers.value
);
</script>
