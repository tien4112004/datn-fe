...
// Rich text various interaction event listeners:
// Disable global hotkeys when focused
// Sync data to vuex when typing text
// Sync rich text state to toolbar when clicking mouse and keyboard
const handleInput = debounce(
  function (isHanldeHistory = false) {
    if (props.value.replace(/ style=\"\"/g, '') === editorView.dom.innerHTML.replace(/ style=\"\"/g, ''))
      return;
    emit('update', {
...
const handleFocus = () => {
  // Don't disable global hotkeys when multiple selection and ctrl or shift key is pressed
...
// Some commands check if current selection is empty before execution, if selection is empty perform select all operation first
...
// Mouse up, execute format brush command
const handleMouseup = () => {
...
// Prosemirror editor initialization and unmounting
onMounted(() => {
